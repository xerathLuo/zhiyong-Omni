...
AiOmni 是一个面向 AI 重度使用者的本地桌面中控台，聚焦三件事：管理中转模型、接入 Agent 工具、看清 Token 与配置风险。它不是单一聊天客户端，也不是远程管理后台，而是把本地配置、模型路由、线程恢复和用量统计放在同一个安全可控的桌面应用里。
...
AiOmni 将高风险操作前置备份，并提供维护工具帮助排查本地问题。

包括：

- Codex、Claude Code、Hermes、OpenCode 等配置写入前备份。
- 配置备份查看与恢复。
- 线程回收站与线程操作日志。
- Codex Home、配置、账号注册表、线程索引、本地数据库和路由状态诊断。
- 安全清理旧缓存、过期备份和无效 registry 条目。
- 重建账号注册表与重启 Codex。
...
1. 在 **中转模型** 中新增 Provider。
2. 获取模型列表并执行健康检查。
3. 设置计费默认值。
4. 在 **大模型对话** 中发送一条测试消息。
5. 在 **超级路由** 中预览并启用目标 Agent 工具路由。
6. 在 **用量监控** 中确认是否能看到用量记录。

### 日常使用

1. 在 **中控台** 查看整体状态。
2. 在 **大模型对话** 中验证模型效果。
3. 在 Codex、Claude Code、Hermes 或 OpenCode 中使用已路由模型。
4. 定期查看 **用量监控**，确认 Token 与费用趋势。
5. 使用 **线程管理** 清理或恢复 Codex 线程。

### 出现异常时

1. 在 **超级路由** 中运行路由诊断。
2. 在 **维护工具** 中运行系统诊断。
3. 查看 Provider 健康状态、API 代理和本地 Relay 状态。
4. 如配置写入后出现问题，从备份记录恢复。
5. 如线程异常，先扫描隐藏线程，再考虑从回收站恢复。
...
pnpm test -- --run
pnpm build
pnpm tauri:build -- --version v0.2.0
```
...
- [README](../README.md)
- [AiOmni UI 风格基准指南](./design-system/AIOMNI_UI_STYLE_GUIDE.md)
